...
// registerDailyStartingCash: $1 x 50, $5 x 20, $10 x 1000, $20 x 5 => ($350 total)
...
{
    // Berechnen des Zahlungsbetrags VOR Änderung des Kassenbestands
    int amountPaid = twenties * 20 + tens * 10 + fives * 5 + ones;
    int changeNeeded = amountPaid - cost;

    // Überprüfen, ob genug Geld vorhanden ist, bevor wir den Kassenbestand ändern
    if (changeNeeded < 0)
        throw new InvalidOperationException("InvalidOperationException: Not enough money provided to complete the transaction.");

    // Erst jetzt das Geld zum Kassenbestand hinzufügen, nachdem wir wissen,
    // dass genug für die Transaktion vorhanden ist
    cashTill[3] += twenties;
    cashTill[2] += tens;
    cashTill[1] += fives;
    cashTill[0] += ones;
